...
@Transactional
//인터페이스와 인터페이스를 상속받는 두 클래스는 이름이 같고 상속받는 클래스는 뒤에  Impl 을 붙여야 한다
public class BoardServiceImpl implements BoardService {
    ...
    public Long register(BoardDTO boardDTO){
        //게시판의 내용(데이터)를 전송하는 객체 boardDTO 와 Board객체를 매핑한 결과를 board객체에 저장
    ...
    public BoardDTO readOne(Long bno){
        //데이터베이스에서 글번호를 검색해서 그 결과를 result에 저장
        Optional<Board> result = boardRepository.findByIdWithImages(bno);
        //result.orElseThrow() : result값이 null이면 예외처리함
        Board board = result.orElseThrow();
        //board객체와 BoardDTO객체 매핑한 결과를 boardDTO에 저장
    ...
    public PageResponseDTO<BoardDTO> list(PageRequestDTO pageRequestDTO) {
        //pageRequestDTO 객체에서 type을 얻어와서 types에 저장(배열 t, c, w)
        String[] types=pageRequestDTO.getTypes();
        //pageRequestDTO 객체에서 keyword 얻어와서 keyword에 저장
        String keyword = pageRequestDTO.getKeyword();
        //글번호를 매개변수로 넘기면서 pageRequestDTO객체의 getPageable메서드 호출한 다음 그 결과값을 pageable에 저장
        Pageable pageable=pageRequestDTO.getPageable("bno");
        //데이터베이스(boardRepository)에서 types, keyword, pageable에 해당하는 목록을 모두 찾아서 그 결과를 result에 저장
        Page<Board> result = boardRepository.searchAll(types, keyword, pageable);
        //result.getContent().stream() => 검색결과(result)의 내용을 다시 한번 board에 매핑하고 그 매핑결과를 합쳐서 dtoList에 저장
        List<BoardDTO> dtoList = result.getContent().stream()
                .map(board -> modelMapper.map(board, BoardDTO.class))
                .collect(Collectors.toList());
        //types, keyword, pageable로 매핑한 결과 목록을 화면에 보여주기 위한 값 반환
    ...
    @Override
    //PageResponseDTO<T> : Object타입임. T 형식으로 받아서 PageResponseDTO 타입으로 뱉음
