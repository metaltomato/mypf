...
public class ReplyController {
    ...
    public PageResponseDTO<ReplyDTO> getList(
            //@PathVariable("bno") : ?bno=197 과 같은 쿼리스트링의 값을 전달받음
    ...
    public ReplyDTO getReplyDTO(
            //@PathVariable("rno") : ?rno=5 와 같은 쿼리스트링의 값을 전달받음
    ...
    public Map<String, Long> modify(
            // ?rno=10 과 같은 쿼리 스트링 만듦
            @PathVariable("rno") Long rno,
            // xml 이나 json 기반의 메시지를 요청할 때 사용
            @RequestBody ReplyDTO replyDTO){
        //댓글 번호를 일치시킴
        replyDTO.setRno(rno);
        //replyService의 modify 메서드 호출
