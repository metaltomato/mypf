...
public class BoardController {
    ...
    //BindingResult : 스프링이 제공하는 검증 오류를 보관하는 객체, 데이터 유효성 검사에서 에러가 발생하면 그 에러 정보를 저장함
    //RedirectAttributes : 서버에 데이터를 전송하면 Redirect발생, 원래 Redirect는 GET방식(데이터 얻어옴)이라 데이터 전송에는 적합하지 않으나,
    //스프링 부트의 RedirectAttributes를 사용하여 GET방식으로도 데이터를 서버에 전송할 수 있다
    ...
    public String modify(PageRequestDTO pageRequestDTO, @Valid BoardDTO boardDTO, BindingResult bindingResult, RedirectAttributes redirectAttributes) {
        ...
        if(bindingResult.hasErrors()){
            //문자열 기록
            log.info("에러 있어요..");
            //pageRequestDTO 객체의 getLink()메서드 호출한 결과를 link에 저장, link="page=3&size=10&type=tcw&keyword자바"
            String link = pageRequestDTO.getLink();
            //bindingResult객체의 모든 에러를 "errors"라는 이름으로 POST방식으로 서버에 전송 -> [[${errors}]]
            redirectAttributes.addFlashAttribute("errors", bindingResult.getAllErrors());
            //데이터의 번호(bno)를 bno라는 이름으로 서버에 POST방식으로 전송
            redirectAttributes.addAttribute("bno", boardDTO.getBno());
            //쿼리스트링 : /board/modify?page=3&size=10&type=tcw&keyword자바 -> 이 주소로 이동함
    ...
    public String remove(BoardDTO boardDTO, RedirectAttributes redirectAttributes) {
        ...
        //boardService객체의 remove 메서드 호출하면서 bno(번호)를 넘김
