...
public class BoardServiceTests {
    ...
    public void testRegisterWithImages(){
        ...
        boardDTO.setFileNames(
                Arrays.asList(
                        UUID.randomUUID()+"^_aa.jpg",
                        UUID.randomUUID()+"^_bb.jpg",
                        UUID.randomUUID()+"^_cc.jpg"
                ));
        Long bno = boardService.register(boardDTO);
        log.info("bno : "+bno);
    }
    ...
    public void testReadAll() {
        ...
        log.info(boardDTO);
        for(String fileNames : boardDTO.getFileNames()){
            log.info(fileNames);
        }
    }
    //게시물 수정(첨부파일 포함)
    ...
    public void testListWithAll() {
        //1번 페이지의 글 목록 100개만 가져와서 pageRequestDTO 에 저장
        PageRequestDTO pageRequestDTO = PageRequestDTO.builder()
                .page(1)
                .size(10)
                .build();
        //페이지 객체 안에 게시글 목록 포함해서 boardService의 listWithAll메서드 호출한 다음 실행 결과를 responseDTO에 저장
        PageResponseDTO<BoardListAllDTO> responseDTO = boardService.listWithAll(pageRequestDTO);
        //responseDTO의 getDtoList() 메서드 호출한 결과를 dtoList 에 저장(첨부파일을 포함한 게시글 목록)
